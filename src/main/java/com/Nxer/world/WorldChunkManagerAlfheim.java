...
public class WorldChunkManagerAlfheim extends WorldChunkManager {
    ...
    public BiomeGenBase getBiomeGenAt(int par1, int par2) {
        ...
        if (biome == null) {
            // FMLLog.warning("[TwilightForest] Suppressing bad biome data in getBiomeGenAt, %s at %d, %d", biome, par1,
            // par2);
    ...
    public float[] getRainfall(float par1ArrayOfFloat[], int par2, int par3, int par4, int par5) {
        ...
        for (int i = 0; i < par4 * par5; i++) {
            ...
            if (ai[i] >= 0 && BiomeGenBase.getBiome(ai[i]) != null) {
                float f = (float) BiomeGenBase.getBiome(ai[i])
                    .getIntRainfall() / 65536F;
