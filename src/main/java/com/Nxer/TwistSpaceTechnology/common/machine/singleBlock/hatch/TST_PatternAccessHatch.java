...
public class TST_PatternAccessHatch extends MTEHatch
	...
	public AEItemStack extractItems(AEItemStack request, Actionable mode, BaseActionSource src) {
		if (getController() == null)
			return null;
		if (getController().mMaxProgresstime > 0)
			return null;
		if (request.getStackSize() > 1) {
			request = (AEItemStack) request.copy().setStackSize(1);
		}
		if (mode == Actionable.SIMULATE) {
			return request;
		}
	...
	private void postChanges() {
		...
		if (lastPattern != null) {
			...
			try {
				...
				this.getProxy().getStorage().postAlterationOfStoredItems(StorageChannel.ITEMS, lastPattern, src);
			} catch (GridAccessException e) {
			}
		}
		...
		if (ctr != null)
			ctr.getInternalPatterns().stream().map(AEItemStack::create).forEach(lastPattern::add);
